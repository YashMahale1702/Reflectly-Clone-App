...
class ThoughtsData with ChangeNotifier {
  List<ImagesModel> _imagesItems = [];

  List<ImagesModel> get imagesItems {
    return [..._imagesItems];
  }

  List<ThoughtsModel> _thoughtsItems = [
    ThoughtsModel(
  ...
  List<ThoughtsModel> get thoughtsItems {
    return [..._thoughtsItems];
  }
  ...
  Future<void> getImageDatas(String query, String page) async {
    ...
    try {
      final List<ImagesModel> items = [];

      final url =
          'https://api.unsplash.com/search/photos.json?&per_page=30&query=$query&client_id=$accessCode&page=1&orientation=portrait&color=blue';
      ...
      extractedData.forEach(
        (cur) {
          items.add(
            ImagesModel(
              source: cur['urls']['regular'],
              blurhash: cur['blur_hash'],
            ),
          );
        },
      );
...
class ImagesModel {
  ...
  ImagesModel({this.source, this.blurhash});
}

class ThoughtsModel {
  final String author;
  final String thought;
  ThoughtsModel({this.author, this.thought});
}
